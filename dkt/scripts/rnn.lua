...
function RNN:rollOutNetwork() 
	...
	for i = 1,self.max_steps do
		self.layers[i]:training() --This sets the mode of the Module (or sub-modules) to train=true
	end
end
...
function RNN:fprop(batch)
	...
	for k = 1,n_steps do
		...
		local output = self.layers[k]:forward(inputs[k]); --pred, err, hidden
		state = output[3]
...
function RNN:calcGrad(batch, rate, alpha)
	local n_steps = getNSteps(batch) -- get max numbers of answers in the data
	local n_students = #batch
  ...
  -- this is the same input when forward is called
